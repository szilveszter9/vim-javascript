...
syntax match   jsAssignmentExpr     /\v%([a-zA-Z_$]\k*\.)*[a-zA-Z_$]\k*\s*\=\>@!/ contains=jsFuncAssignExpr,jsAssignExpIdent,jsPrototype,jsStrictEqual,jsOperator,jsThis,jsNoise,jsArgsObj
...
syntax cluster jsAll        contains=@jsExpression,jsLabel,jsConditional,jsRepeat,jsReturn,jsStatement,jsTernaryIf,jsStrictEqual,jsException
syntax region  jsBracket    matchgroup=jsBrackets     start="\[" end="\]" contains=@jsAll,jsParensErrB,jsParensErrC,jsBracket,jsParen,jsBlock,@htmlPreproc fold
syntax region  jsParen      matchgroup=jsParens       start="("  end=")"  contains=@jsAll,jsOf,jsParensErrA,jsParensErrC,jsParen,jsBracket,jsStrictEqual,jsBlock,@htmlPreproc fold
...
exe 'syntax match jsStrictEqual /\(===\)/ contained '.(exists('g:javascript_conceal_strictEqual') ? 'conceal cchar='.g:javascript_conceal_strictEqual : '')
